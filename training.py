...
# Load dataset
# -----------------------------
DATA_PATH = "processed_kinase_with_fingerprints.csv"
df = pd.read_csv(DATA_PATH)

df = df[["canonical_smiles", "pchembl_value", "fingerprint"]].dropna()
df.rename(columns={"canonical_smiles": "smiles", "pchembl_value": "pIC50"}, inplace=True)

# Convert fingerprint string → numpy array
def fix_fp(x):
    if isinstance(x, str):
